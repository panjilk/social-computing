...
共 6 次运行，三情景使用相同连续种子。无国内改善、公告、材料帮助或预防机制；关闭新争议，固定初始争议队列。
所有参数为探索值。渠道效果、成本和时间不相同但固定，三组只切换可用渠道；因此结果不能视为某类制度天然更优。

|情景|国内使用|客户使用|同时或先后使用两者|最终解决|未解决|
|---|---:|---:|---:|---:|---:|
|仅国内渠道|100.00%|0.00%|0.00%|26.34%|73.66%|
|仅客户渠道|0.00%|61.16%|0.00%|10.27%|89.73%|
|双渠道共存|96.43%|51.79%|48.21%|29.91%|70.09%|

共存组：国内→客户占全部案件 37.95%，客户→国内占全部案件 3.57%。
条件追加比例：国内→客户 / 首次仅国内 = 46.36%；客户→国内 / 首次仅客户 = 50.00%。
